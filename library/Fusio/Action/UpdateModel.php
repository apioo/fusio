...
class UpdateModel extends ModelActionAbstract
{
	public function execute(Request $request, array $parameters)
	{
		...
		// insert record
		return $this->updateRecord($this->getConnector($connectionId), $table, $record);
	}
	...
	protected function updateRecord(ConnectorInterface $connector, $table, RecordInterface $record)
	{
		try {
			$connector->update($table, $record);
