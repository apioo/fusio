...
class BankAccountController extends DanapayController
{
    public function handle(RequestInterface $request, ParametersInterface $configuration, ContextInterface $context): mixed {
        ...
        $res = $client->request('GET', '/api/v1/userBankAccounts/all', [
            'headers' => 
                [
                    'X-API-SECRET' => getenv('DANAPAY_API_SECRET'),
        ...
        foreach($ret->data as $account) {
            $country = null;
            foreach(Library::COUNTRIES as $c) {
                if($c['name'] == $account->country) {
                    $country = $c['code'];
                    break;
                }  
            }
